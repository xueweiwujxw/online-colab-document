...
- Sheets 已改为从固定上游提交及递归子模块构建，镜像内补丁会保留宿主身份、脱敏上游请求日志，并将 WOPI 会话令牌传至经 Go 权限代理的协作 WebSocket。实际双用户浏览器验证已确认宿主 xlsx 工作表加载、两端真实显示名、持续 WebSocket 连接、远程名称、远程选区，以及 owner 公式栏写入后 editor 在同一单元格读到相同值；viewer、断线重连及保存/版本/审计仍待验收。
