...
补充记录（2026-08-07）：管理员控制台已扩展为后台入口：可修改本地账号资料、管理账号角色/禁用状态、查看不含 secret 的 OIDC 运行配置、筛选 MinIO 对象、列出并软删除全局文档。OIDC 配置仍只从受控部署环境变量读取，管理页不会读取或写入 client secret；更新后端环境变量后需重启服务。

补充记录（2026-08-07）：管理后台重构为侧边导航工作台，概览、用户、文件、对象存储、OIDC、审计日志分离为独立入口。审计日志增加操作/对象/操作者/IP/时间范围组合筛选、清除筛选、展开详情与分页加载；MinIO 增加路径浏览、当前结果搜索、排序、目录入口、下载和复制对象路径。高风险删除仍保留确认与审计。
...
## RustFS 存储替换（2026-09-17）

当前默认存储由 MinIO 替换为 RustFS 1.0.0；上文的 MinIO 部署记录为历史方案。保留 S3_* 环境变量及 S3 SDK，通过独立 rustfs-data 卷部署；旧数据须经 S3 API 复制，禁止直接复用 MinIO 数据卷。验证范围包含对象读写、预签名、列表、删除、启动失败重试，以及文档版本/权限回归。
